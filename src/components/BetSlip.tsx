...
export const BetSlip = React.memo(function BetSlip({ onPlaceBet }: { onPlaceBet?: (bets: BetSlipItem[], stake: number, type: 'single' | 'express' | 'system') => void }) {
  ...
  return (
    <Sheet open={betSlipOpen} onOpenChange={setBetSlipOpen}>
      ...
      <SheetContent className="bg-background border-border text-foreground w-full sm:max-w-md p-0 flex flex-col">
        ...
        {betSlip.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-muted-foreground mb-1">Купон пуст</p>
            <p className="text-xs text-muted-foreground">Нажмите на коэффициент, чтобы добавить ставку</p>
          </div>
        ) : (
          <>
            {/* Bet type tabs */}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-medium text-foreground truncate">{item.matchTitle}</p>
                      </div>
                      <button
                        onClick={() => removeBet(item.id)}
                        className="text-muted-foreground hover:text-red-400 shrink-0 p-0.5"
                      >
              ...
              <div className="grid grid-cols-3 gap-2">
                {activeTab === 'single' && betSlip.length > 1
                  ? betSlip.map((item) => {
                      ...
                      return (
                        <div key={item.id} className="bg-muted/50 rounded-lg p-2 text-center">
                          <p className="text-[10px] text-muted-foreground truncate">{item.prediction}</p>
                          <p className="text-sm font-bold text-emerald-400">{item.odds.toFixed(2)}</p>
                          <p className="text-[10px] text-foreground">+{singleProfit.toFixed(0)} ₽</p>
                        </div>
                      );
                    })
                  : (
                    <>
                      <div className="bg-muted/50 rounded-lg p-2 text-center">
                        <p className="text-[10px] text-muted-foreground">Коэфф.</p>
                        <p className="text-sm font-bold text-emerald-400">
                          {activeTab === 'single' ? betSlip[0]?.odds.toFixed(2) : totalOdds.toFixed(2)}
                        </p>
                      </div>
                      <div className="bg-muted/50 rounded-lg p-2 text-center">
                        <p className="text-[10px] text-muted-foreground">Выплата</p>
                        <p className="text-sm font-bold text-foreground">{potentialWin.toFixed(0)} ₽</p>
                      </div>
                      <div className="bg-muted/50 rounded-lg p-2 text-center">
