...
export function CookieBanner() {
  ...
  const dismiss = () => {
    // Close without consent - banner will reappear on next visit
    setVisible(false);
  };
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed bottom-16 lg:bottom-4 left-4 right-4 lg:left-auto lg:right-4 lg:max-w-md z-50 bg-card border border-border rounded-xl shadow-2xl shadow-black/50 p-4"
        >
          <div className="flex items-start gap-3">
            ...
            <button onClick={dismiss} aria-label="Закрыть баннер cookie" className="text-muted-foreground hover:text-muted-foreground shrink-0">
              <X className="w-4 h-4" />
            </button>
          </div>
        </motion.div>
