...
const tierConfig = {
  ...
  free: { color: 'text-muted-foreground', bg: 'bg-gray-500/20', border: 'border-gray-500/30', icon: <Zap className="w-3 h-3" /> },
};
...
const rankConfig: Record<number, { color: string; bg: string; icon: React.ReactNode }> = {
  ...
  2: { color: 'text-muted-foreground', bg: 'bg-gradient-to-br from-gray-400/20 to-gray-500/10', icon: <Medal className="w-5 h-5" /> },
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-foreground">Лидерборд</h2>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {top3.map((user) => {
          const config = rankConfig[user.rank] || { color: 'text-muted-foreground', bg: 'bg-muted/50', icon: null };
          return (
            <motion.div
              key={user.id}
              ...
              className={`${config.bg} rounded-xl border border-border p-4 text-center relative overflow-hidden ${user.rank === 1 ? 'ring-2 ring-yellow-500/30 sm:scale-105' : ''}`}
            >
              {user.rank === 1 && (
              ...
              <div className="relative">
                ...
                <div className="mt-3 space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">Проход</span>
      ...
      <div className="space-y-2">
        {rest.map((user, index) => (
          ...
          >
            <Card className="bg-muted/40 border-border hover:border-border/50 transition-all">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-4 text-xs shrink-0">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-foreground font-bold">+{(user.profit / 1000).toFixed(1)}K</p>
                    </div>
                  </div>
                </div>
              </CardContent>
      ...
      <div className="mt-6 grid grid-cols-3 gap-3">
        ...
        <div className="bg-muted/30 rounded-xl p-3 border border-border text-center">
          ...
          <p className="text-sm font-bold text-foreground">+17.3%</p>
          <p className="text-[10px] text-muted-foreground">Средний ROI</p>
        </div>
        <div className="bg-muted/30 rounded-xl p-3 border border-border text-center">
          <Users className="w-5 h-5 text-yellow-400 mx-auto mb-1" />
          <p className="text-sm font-bold text-foreground">1.2M ₽</p>
          <p className="text-[10px] text-muted-foreground">Общий профит</p>
        </div>
      </div>
    </section>
  );
